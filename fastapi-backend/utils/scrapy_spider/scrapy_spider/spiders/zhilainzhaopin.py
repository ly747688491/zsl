...
from utils.scrapy_spider.setting import COOKIE, HEADERS
...
class ZhilainzhaopinSpider(scrapy.Spider):
    ...
    def parse(self, response: Response, **kwargs: Any) -> Any:
        ...
        for city_name in self.city_list.keys():
            city_url = f"https://sou.zhaopin.com/?jl={self.city_list[city_name]}&kw={keyword_encoded}&p=1"
            yield scrapy.Request(
                url=city_url,
    ...
    def parse_page(self, response: Response, **kwargs: Any) -> Any:
        ...
        for body in list_body:
            item["job_name"] = (
                body.xpath(".//div[@class='jobinfo']/div/span/text()")
                .extract_first()
                .strip()
            )
            item["job_education"] = (
                body.xpath(".//div[@class='jobinfo__other-info']/div[3]/text()")
                .extract_first()
                .strip()
            )
            item["job_city"] = (
                body.xpath(".//div[@class='jobinfo__other-info']/div[1]/span/text()")
                .extract_first()
                .strip()
            )
            item["job_experience"] = (
                body.xpath(".//div[@class='jobinfo__other-info']/div[2]/text()")
                .extract_first()
                .strip()
            )
            try:
            ...
            item["company_name"] = (
                body.xpath(
                    "//div[@class='companyinfo__top']/div[@class='companyinfo__name']/text()"
                )
                .extract_first()
                .strip()
            )
